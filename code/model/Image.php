...
class Image extends File {
	...
	public function SetWidth($width) {
			...
			// checking if the original is the same size as the requested setWidth is pointless anyway...
	...
	public function getFormattedImage($format, $arg1 = null, $arg2 = null) {
			...
			if($this->ID && $this->Filename) {
				...
				$remote = "http://" . S3_BUCKET . ".s3.amazonaws.com/" . $cacheFile;
				$exists = strstr(current(get_headers($remote)), "200");
				//$exists = false;
				
				if(!$exists || isset($_GET['flush'])) {
	...
	function generateFormattedImage($format, $arg1 = null, $arg2 = null) {
		...
		if (strstr(current(get_headers($this->S3URL)), "200")) {		
		$gd = new GD($this->S3URL);
				if($gd->hasGD()){
	...
	public function WriteS3Cache($cacheFile) {
		$fileName = Director::baseFolder()."/" . $cacheFile;
		if($fileName){
			$s3Name = $cacheFile;
			$bucket = $this->getUploadBucket();
			$S3 = new S3();
			$S3->putObjectFile($fileName, $bucket, $s3Name, S3::ACL_PUBLIC_READ);
			//$this->deleteFormattedImages(); <- THIS DELETES ALL THE TINY_MCE RESAMPLED IMAGES?!		
		}
	}
	 ...
	 * @return string|int
	 * RUNNING THIS ON LARGE FILES TAKES AGES! BEST TO NOT EVEN BOTHER
	 */
	function getDimensions($dim = "string") {
			if($this->FileName && $this->S3URL) {
				$imagefile = $this->S3URL;
				$exists = strstr(current(get_headers($imagefile)), "200");
				if($exists) {
					try {
	 ...
	 * WHEN USING S3 THIS IS VERY SLOW - BEST TO SAVE AS META DATA ON UPLOAD IF YOU WANT OT USE THIS
	 * @return int
	 */
