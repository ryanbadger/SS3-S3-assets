...
class File extends DataObject {
			...
			CompositeField::create(
				CompositeField::create(
					...
					$urlField = new ReadonlyField('ClickableURL', _t('AssetTableField.URL','URL'),
						sprintf('<a href="%s" target="_blank">%s</a>', $this->Link(), $this->S3URL)
					),
	...
	public function getURL() {
		...
		if (defined('S3_CDN')) {
			return S3_CDN . $this->getFilename();	
		} else {
			return "//" . S3_BUCKET . ".s3.amazonaws.com/" . $this->getFilename();
		}
	}
	...
	public function WriteS3File() {
		if(!is_a($this, 'Folder') && $this->Filename) {
			$s3Name = $this->Filename;
			
			$remote = "http://" . S3_BUCKET . ".s3.amazonaws.com/{$s3Name}";
			$exists = strstr(current(get_headers($remote)), "200");
			$input = $this->getFullPath();
			$local = file_exists($input);
			if(!$exists && $local){
				$S3 = new S3();
				$S3->putObjectFile($input, S3_BUCKET, $s3Name, S3::ACL_PUBLIC_READ);
			}
			$this->S3URL = "http://" . S3_BUCKET . ".s3.amazonaws.com/{$s3Name}";
